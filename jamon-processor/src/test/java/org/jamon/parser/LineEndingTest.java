...
public class LineEndingTest extends AbstractParserTest {
  ...
  @Test
  public void testBackslashForUnix() throws Exception {
    assertEquals(
      topNode().addSubNode(new TextNode(location(1, 1), "textmore")),
      parse("text\\\nmore"));
  }

  @Test
  public void testNonNewlineBackslash() throws Exception {
    assertEquals(
      topNode().addSubNode(new TextNode(location(1, 1), "text\\\rmore")),
      parse("text\\\rmore"));
    assertEquals(
      topNode().addSubNode(new TextNode(location(1, 1), "text\\more")),
      parse("text\\more"));
    assertEquals(
      topNode().addSubNode(new TextNode(location(1, 1), "text\\")),
      parse("text\\"));
  }

  @Test
  public void testBackslashForWindows() throws Exception {
    assertEquals(
      topNode().addSubNode(new TextNode(location(1, 1), "textmore")),
      parse("text\\\r\nmore"));
  }
